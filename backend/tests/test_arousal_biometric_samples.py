...
through POST /api/v1/samples/batch.
"""
...
async def _get_arousal(client: AsyncClient, checkin_id: int) -> Response:
    return await client.get(f"/api/v1/checkins/{checkin_id}/arousal")
...
async def test_user_with_200_samples_and_2_checkins_gets_source_samples(
    ...
    await _create_checkin(client, user_id, heart_rate=75)
    checkin2 = await _create_checkin(client, user_id, heart_rate=80)

    baseline_resp = await client.get(f"/api/v1/user-baseline/{user_id}/heart_rate")
    assert baseline_resp.status_code == 200
    baseline = baseline_resp.json()
    assert baseline["source"] == "samples"
    assert baseline["sample_n"] == 200

    arousal = (await _get_arousal(client, checkin2["id"])).json()
...
async def test_user_with_0_samples_scores_from_checkins_exactly_as_before(
    ...
    for hr in _TEN_HR_VALUES:
        checkin = await _create_checkin(client, user_id, heart_rate=hr)

    arousal = (await _get_arousal(client, checkin["id"])).json()
...
async def test_window_average_used_when_at_least_3_samples_in_window(
    ...
    spike = await _create_checkin(client, user_id, heart_rate=70)
    arousal = (await _get_arousal(client, spike["id"])).json()
...
async def test_fewer_than_3_samples_in_window_falls_back_to_typed_checkin_value(
    ...
    checkin = await _create_checkin(client, user_id, heart_rate=70)
    arousal = (await _get_arousal(client, checkin["id"])).json()
...
async def test_sign_elevated_heart_rate_samples_sourced_raises_score_above_half(
    ...
    await _create_checkin(client, user_id, heart_rate=75)  # triggers refresh_baseline
    ...
    spike = await _create_checkin(client, user_id, heart_rate=75)
    arousal = (await _get_arousal(client, spike["id"])).json()
...
async def test_sign_elevated_hrv_samples_sourced_lowers_score_below_half(
    ...
    await _create_checkin(client, user_id, hrv_ms=51)  # triggers refresh_baseline

    baseline_resp = await client.get(f"/api/v1/user-baseline/{user_id}/hrv_ms")
    assert baseline_resp.json()["source"] == "samples"

    spike_readings = [(_minutes_ago(m, now=now), 200.0) for m in (1, 2, 3)]
    await _post_samples(client, user_id, "hrv_ms", spike_readings)

    spike = await _create_checkin(client, user_id, hrv_ms=51)
    arousal = (await _get_arousal(client, spike["id"])).json()
...
async def test_baseline_factor_1_0_for_samples_source_with_100_or_more_samples(
    ...
    checkin = await _create_checkin(client, user_id, heart_rate=75)
    arousal = (await _get_arousal(client, checkin["id"])).json()
