...
async def test_sign_elevated_heart_rate_raises_score_above_half(
    ...
    spike = await _create_checkin(client, user_id, heart_rate=150)
...
async def test_sign_elevated_hrv_lowers_score_below_half(
    ...
    for hrv in (50, 52, 48, 51, 49, 50, 52, 48, 51, 50):
        await _create_checkin(client, user_id, hrv_ms=hrv)

    spike = await _create_checkin(client, user_id, hrv_ms=200)
    arousal = (await _get_arousal(client, spike["id"])).json()
...
async def test_identical_readings_skip_metric_no_divide_by_zero(
    ...
    arousal_resp = await _get_arousal(client, checkin["id"])
...
async def test_single_metric_checkin_has_metrics_used_one_and_low_confidence(
    ...
    checkin = await _create_checkin(client, user_id, heart_rate=85)
    arousal = (await _get_arousal(client, checkin["id"])).json()
...
async def test_arousal_404_when_no_checkin(client: AsyncClient) -> None:
    resp = await _get_arousal(client, 99_999_999)
