"""add versioned per-user normalization snapshots
...
def upgrade() -> None:
    ...
    # Give every pre-existing user the same population snapshot that new
    # registrations receive. Physiological zeros are explicit placeholders.
    op.execute(
        """
...
def downgrade() -> None:
    op.drop_index(
        "ix_user_normalization_snapshots_user_id_recorded_at",
        table_name="user_normalization_snapshots",
    )
    op.drop_table("user_normalization_snapshots")
